...
export function useWETH9Contract(
  ...
  return useContract(
    chainId && WNATIVE_ADDRESS[chainId],
    WETH9_ABI,
...
export function useENSRegistrarContract(
  ...
  const { chainId } = useActiveWeb3React();
  return useContract(
    chainId && ENS_REGISTRAR_ADDRESS[chainId],
    ENS_ABI,
    withSignerIfPossible,
  );
}
...
export function useProtocolMerkleDistributorContract(): Contract | null {
  const { chainId } = useActiveWeb3React();
  return useContract(
    chainId ? '0x1026cbed7b7E851426b959BC69dcC1bf5876512d' : undefined,
    MERKLE_DISTRIBUTOR_ABI,
    true,
  );
}
...
export function useChainlinkOracle(): Contract | null {
  const { chainId } = useActiveWeb3React();
  return useContract(
    chainId && CHAINLINK_ORACLE_ADDRESS[chainId],
